...
from opensora_base.opensora.acceleration.communications import all_to_all, split_forward_gather_backward
from opensora_base.opensora.acceleration.parallel_states import get_sequence_parallel_group
from opensora_base.opensora.models.sparse_engine import sparse_tensor
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.do_sparse and N < B:
            reduce, unreduce = sparse_tensor(x, 19, 1, 2, 1, 0.98, "cosine")
            #pdb.set_trace()
            x = reduce(x)
            qkv = self.qkv(x)
            #pdb.set_trace()
            ...
            reduce, unreduce = sparse_tensor(x, 27, 15, 2, 1, 0.98, "cosine")
            x = reduce(x)
            qkv = self.qkv(x)
            qkv = unreduce(qkv)
        else:
...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, d_model, num_heads, do_sparse: bool = False, attn_drop=0.0, proj_drop=0.0):
    ...
    def forward(self, x, cond, mask=None):
        ...
        B, N, C = x.shape
        if self.do_sparse:
            reduce, unreduce = sparse_tensor(x, 27, 285, 2, 2, 1, "cosine")
            x = reduce(x)
            q = self.q_linear(x)
            q = unreduce(q)
